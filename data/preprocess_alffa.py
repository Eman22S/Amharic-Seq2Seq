...
parser = argparse.ArgumentParser(description='Preprocess program for Alffa dataset.')
parser.add_argument('--data_path', type=str, help='Path to raw Alffa dataset')
...
if paras.target == 'subword':
    # Setup Path
    output_dir = os.path.join(paras.output_path,'_'.join(['Alffa',str(paras.feature_type)+str(dim),str(paras.target)+str(paras.n_tokens)]))
...
for s in tr_set:
    ...
    if output_dir is None:
        output_dir = os.path.join(paras.output_path,'_'.join(['Alffa',str(paras.feature_type)+str(dim),str(paras.target)+str(len(encode_table))]))
    if not os.path.exists(output_dir):os.makedirs(output_dir)
