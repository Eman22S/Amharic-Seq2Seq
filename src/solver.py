...
class Tester(Solver):
    ...
    def exec_file(self,file):
        self.decode_beam_size = self.config['solver']['decode_beam_size']
        self.verbose("Predicting the Amharic text of the filename " + file)
        x = torch.FloatTensor(np.load(file))
        x = pad_sequence(x, batch_first=True)
        mfcc_len = x.shape[0]
        x = np.reshape(x, (1, mfcc_len, 80))
        self.beam_decode(x)

    def write_hyp(self,hyps,y=None):
    ...
    def beam_decode(self,x,y=None):
        ...
        if y is not None :self.write_hyp(hyps,y)
        else:self.write_hyp(hyps)
        del hyps
        return 1
